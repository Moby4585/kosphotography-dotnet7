...
namespace kosphotography
{
    public class PhotoBitmap : IBitmap
    {
        ...
        public SKBitmap bitmapGrayscale;
        ...
        public PhotoBitmap()
        {
            bitmapGrayscale = new SKBitmap(width, height);
        ...
        {
            SKColor col = bitmapGrayscale.GetPixel(Math.Min(x, bitmapGrayscale.Width - 1), Math.Min(y, bitmapGrayscale.Height - 1));
            ...
            float strength = remap(Math.Min(((float)col.Red)/* * 6f*/, 255f), minimumBrightness, maximumBrightness, 0, 255);
            ...
            return new SKColor(Convert.ToUInt32(color));
        }
            ...
            if (isHeightEdge)
            {
                ...
                for (int y = 0; y < height; y++)
                {
                    for (int x = 0; x < height; x++)
                    {
                        ...
                        byte brightness = (byte)(bmp.GetPixel(x + deadshift, y).Blue / (byte)2);
