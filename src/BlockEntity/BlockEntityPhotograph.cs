...
namespace kosphotography
{
    class BlockEntityPhotograph : BlockEntityContainer
    {
        ...
        private void genPhoto()
        {
            ...
            SKBitmap bmp = BitmapUtil.GrayscaleBitmapFromPixels(Encoding.GetEncoding(28591).GetBytes(attributes.GetString("photo", "")), attributes.GetInt("width", 0), attributes.GetInt("height", 0));

            //var graphics = Graphics.FromImage(bmp);
            //graphics.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.NearestNeighbor;
            //graphics.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.None;

            //bmp = new BitmapExternal(bmp, new Size(bmp.Width / (bmp.Height / (int)Math.Pow(2, KosPhotographyConfig.Current.PhotographLod)), (int)Math.Pow(2, KosPhotographyConfig.Current.PhotographLod)));

            SKImageInfo resizeInfo = new()
            {
                Width = bmp.Width / (bmp.Height / (int)Math.Pow(2, KosPhotographyConfig.Current.PhotographLod)),
                Height = (int)Math.Pow(2, KosPhotographyConfig.Current.PhotographLod),
                ColorType = SKColorType.Gray8
            };

            bmp = bmp.Resize(resizeInfo, SKFilterQuality.High);
        ...
        /*public SKBitmap GrabScreenshot(ICoreClientAPI capi, bool scaleScreenshot, bool flip, bool withAlpha)
        {
            ...
            SKBitmap bmp = new SKBitmap(capi.Render.FrameWidth, capi.Render.FrameHeight);
